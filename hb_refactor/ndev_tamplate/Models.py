...
__all__ = ['layer_types', 'extract_layer', 'FlexConvLayer', 'FlexUnetEncoder', 'FlexUnetDecoder', 'SUNET']
...
class FlexUnetDecoder(nn.Module):
    '''Creates flexible encoder for Unets
       ...
       \n`conv_depth`: number of conv layers
       '''
    def __init__(self, nf,  ks, st, pd, conv_depth, ndim=3,  act_fn=nn.ELU()):
        super().__init__()
        nf = self._get_enc_filter(nf, conv_depth)
        self.module_dict = nn.ModuleDict()

        for i in range(conv_depth):
            self.module_dict[f'conc_{i}'] = nn.Sequential(FlexConvLayer(nf, nf//2, ks=ks, st=st, ups = True, act_fn=act_fn, pd=pd, ndim=ndim))
            self.module_dict[f'pass_{i}'] = nn.Sequential(FlexConvLayer(nf, nf//2, ks=ks, st=st, act_fn=act_fn, ndim=ndim),
                                                          FlexConvLayer(nf//2, nf//2, ks=ks, st=st, act_fn=act_fn, ndim=ndim))
            nf //=2
...
class SUNET(nn.Module):
    '''General Encoder for 2D or 3D
       ...
       \n`conv_depth`: number of conv layers
    '''
    def __init__(self, ni, nc, ks, st, pd, conv_depth, ndim):
        super().__init__()
        self.encoder = FlexUnetEncoder(ni, nc, ks, st, pd, conv_depth, ndim)
        self.decoder = FlexUnetDecoder(nc, ks, st, pd+1, conv_depth, ndim)
    def forward(self, x):
        x = self.encoder(x)
        x = self.decoder(*x)
        return x

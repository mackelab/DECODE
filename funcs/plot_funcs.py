...
def create_3d_hist(preds, z_clip=None, pix_size=5, sigma=3, contrast_fac=10, clip_density=100): 
    ...
    storm_hsv = np.concatenate([storm_hue[:,None],np.ones_like(storm_hue)[:,None],np.ones_like(storm_hue)[:,None]],1)
    cmap = hsv_to_rgb(storm_hsv)   
    ...
    sr_RGBblur = cv2.GaussianBlur(sr_RGB,(3,3),sigma/pix_size)

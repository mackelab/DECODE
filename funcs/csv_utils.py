...
def write_csv(pred_list, name):
    ...
    with open(name, 'w') as csvfile:
        ...
        csvwriter.writerow(['Ground-truth', 'frame', 'xnano', 'ynano', 'znano', 'intensity','x_sig','y_sig','z_sig'])
        for p in pred_list:
...
def crop_preds(preds,sl=None,lims=None,z_lim=[-np.inf,np.inf], pix_nm=[100,100]):
    ...
    inds = np.argwhere((preds[:,1] > i_l) & (preds[:,1] < i_h) & (preds[:,3] > y_l) & (preds[:,3] < y_h) & (preds[:,2] > x_l) & (preds[:,2] < x_h) & (preds[:,4] > z_lim[0]) & (preds[:,4] < z_lim[1]))[:,0]
    preds = preds[inds]
    preds[:,2] -= x_l
    preds[:,3] -= y_l
